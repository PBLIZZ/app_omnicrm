...
class UserService extends BaseService<User, CreateUserDto, UpdateUserDto> {
  ...
  private isPaginationOptions(arg?: UserFilter | PaginationOptions): arg is PaginationOptions {
...
abstract class Repository<T extends BaseEntity, K extends keyof T = "id"> {
  ...
  async createWithValidation(entity: Omit<T, K | "createdAt" | "updatedAt">): Promise<T> {

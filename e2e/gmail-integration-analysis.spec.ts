...
test.describe("Gmail Integration Analysis - Broken Existing Flows", () => {
  ...
  test("Dashboard Manual Sync - Documents broken flow", async ({ page }) => {
    ...
    if (await gmailSwitch.isEnabled()) {
      ...
      const previewSection = page.locator('[data-testid="preview"], .space-y-4:has-text("Preview")');
  ...
  test("Settings Sync Preferences - Documents old component issues", async ({ page }) => {
    ...
    const connectionCard = page.locator('[data-testid="connection-status"], .space-y-3:has-text("Google Connected")');
    await expect(connectionCard).toBeVisible();
    
    // Check for configuration sections
    const gmailSection = page.locator('[data-testid="gmail-config"], .space-y-4:has-text("Gmail")');
    const calendarSection = page.locator('[data-testid="calendar-config"], .space-y-4:has-text("Calendar")');
  ...
  test("Dashboard Connect Data - Documents google_not_connected errors", async ({ page }) => {
    ...
    const gmailCard = page.locator('.space-y-6:has-text("Email Integration"), .hover\\:shadow-md:has-text("Gmail")');
    ...
    if (await syncButton.count() > 0) {
      ...
      const statusBadge = gmailCard.locator('.bg-green-50, .bg-red-50, .text-green-700, .text-red-700');

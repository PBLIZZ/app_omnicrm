import { and, desc, eq, gte, ilike, inArray, lte, count, sql } from "drizzle-orm";

import { inboxItems, type CreateInboxItem, type InboxItem } from "@/server/db/schema";
import type { DbClient } from "@/server/db/client";

type InboxItemStatus = "unprocessed" | "processed" | "archived";

export interface InboxListParams {
  status?: InboxItemStatus[];
  search?: string;
  createdAfter?: Date;
  createdBefore?: Date;
  page?: number;
  pageSize?: number;
  sort?: "createdAt" | "updatedAt";
  order?: "asc" | "desc";
}

export class InboxRepository {
  constructor(private readonly db: DbClient) {}

  async listInboxItems(
    userId: string,
    params: InboxListParams = {},
  ): Promise<{ items: InboxItem[]; total: number }> {
    const page = Math.max(params.page ?? 1, 1);
    const pageSize = Math.min(Math.max(params.pageSize ?? 50, 1), 200);
    const offset = (page - 1) * pageSize;
    const sortKey = params.sort ?? "createdAt";
    const sortDir = params.order === "asc" ? "asc" : "desc";

    const conditions = [eq(inboxItems.userId, userId)];

    if (params.status?.length) {
      conditions.push(inArray(inboxItems.status, params.status));
    }

    if (params.search) {
      conditions.push(ilike(inboxItems.rawText, `%${params.search}%`));
    }

    if (params.createdAfter) {
      conditions.push(gte(inboxItems.createdAt, params.createdAfter));
    }

    if (params.createdBefore) {
      conditions.push(lte(inboxItems.createdAt, params.createdBefore));
    }

    // Count total
    const countResult = await this.db
      .select({ count: count() })
      .from(inboxItems)
      .where(and(...conditions));

    const totalRow = countResult[0];
    const total = totalRow?.count ?? 0;

    // Fetch items
    const sortColumnMap = {
      createdAt: inboxItems.createdAt,
      updatedAt: inboxItems.updatedAt,
    } as const;

    const rows = await this.db
      .select()
      .from(inboxItems)
      .where(and(...conditions))
      .orderBy(sortDir === "desc" ? desc(sortColumnMap[sortKey]) : sortColumnMap[sortKey])
      .limit(pageSize)
      .offset(offset);

    return { items: rows, total };
  }

  async getInboxItem(userId: string, itemId: string): Promise<InboxItem | null> {
    const [row] = await this.db
      .select()
      .from(inboxItems)
      .where(and(eq(inboxItems.userId, userId), eq(inboxItems.id, itemId)))
      .limit(1);

    return row ?? null;
  }

  async createInboxItem(data: CreateInboxItem & { userId: string }): Promise<InboxItem> {
    const [newItem] = await this.db
      .insert(inboxItems)
      .values({
        userId: data.userId,
        rawText: data.rawText,
        status: "unprocessed",
        createdTaskId: null,
        processedAt: null,
      })
      .returning();

    if (!newItem) {
      throw new Error("Failed to create inbox item - no data returned");
    }

    return newItem;
  }

  async updateInboxItem(
    userId: string,
    itemId: string,
    data: Partial<CreateInboxItem>,
  ): Promise<InboxItem | null> {
    const updateValues: Record<string, unknown> = {
      updatedAt: new Date(),
    };

    // No business logic - just save what service layer provides
    if (data.status !== undefined) {
      updateValues["status"] = data.status;
    }

    if (data.processedAt !== undefined) {
      updateValues["processedAt"] = data.processedAt;
    }

    if (data.createdTaskId !== undefined) {
      updateValues["createdTaskId"] = data.createdTaskId ?? null;
    }

    const [updatedItem] = await this.db
      .update(inboxItems)
      .set(updateValues)
      .where(and(eq(inboxItems.userId, userId), eq(inboxItems.id, itemId)))
      .returning();

    return updatedItem ?? null;
  }

  async deleteInboxItem(userId: string, itemId: string): Promise<boolean> {
    const deleted = await this.db
      .delete(inboxItems)
      .where(and(eq(inboxItems.userId, userId), eq(inboxItems.id, itemId)))
      .returning({ id: inboxItems.id });

    return deleted.length > 0;
  }

  async bulkUpdateStatus(
    userId: string,
    itemIds: string[],
    status: InboxItemStatus,
  ): Promise<InboxItem[]> {
    if (itemIds.length === 0) {
      return [];
    }

    const updatedItems = await this.db
      .update(inboxItems)
      .set({
        status,
        updatedAt: new Date(),
        ...(status === "processed" ? { processedAt: new Date() } : {}),
      })
      .where(and(eq(inboxItems.userId, userId), inArray(inboxItems.id, itemIds)))
      .returning();

    return updatedItems;
  }

  async bulkDeleteInboxItems(userId: string, itemIds: string[]): Promise<number> {
    if (itemIds.length === 0) {
      return 0;
    }

    const [{ n = 0 } = {}] = await this.db
      .select({ n: sql<number>`count(*)` })
      .from(inboxItems)
      .where(and(eq(inboxItems.userId, userId), inArray(inboxItems.id, itemIds)))
      .limit(1);

    if (n === 0) {
      return 0;
    }

    await this.db
      .delete(inboxItems)
      .where(and(eq(inboxItems.userId, userId), inArray(inboxItems.id, itemIds)));

    return n;
  }

  async getInboxStats(
    userId: string,
  ): Promise<{ unprocessed: number; processed: number; archived: number; total: number }> {
    const rows = await this.db
      .select({
        status: inboxItems.status,
        count: sql<number>`count(*)`,
      })
      .from(inboxItems)
      .where(eq(inboxItems.userId, userId))
      .groupBy(inboxItems.status);

    const stats = {
      unprocessed: 0,
      processed: 0,
      archived: 0,
      total: 0,
    } satisfies Record<InboxItemStatus | "total", number>;

    for (const row of rows) {
      const status = (row.status ?? "unprocessed") as InboxItemStatus;
      const count = row.count;
      stats[status] = count;
      stats.total += count;
    }

    return stats;
  }

  async getUnprocessedItems(userId: string, limit = 10): Promise<InboxItem[]> {
    const rows = await this.db
      .select()
      .from(inboxItems)
      .where(and(eq(inboxItems.userId, userId), eq(inboxItems.status, "unprocessed")))
      .orderBy(desc(inboxItems.createdAt))
      .limit(limit);

    return rows;
  }

  async markAsProcessed(
    userId: string,
    itemId: string,
    createdTaskId?: string,
  ): Promise<InboxItem | null> {
    const [updatedItem] = await this.db
      .update(inboxItems)
      .set({
        status: "processed",
        processedAt: new Date(),
        updatedAt: new Date(),
        ...(createdTaskId ? { createdTaskId } : {}),
      })
      .where(and(eq(inboxItems.userId, userId), eq(inboxItems.id, itemId)))
      .returning();

    return updatedItem ?? null;
  }
}

export function createInboxRepository(db: DbClient): InboxRepository {
  return new InboxRepository(db);
}

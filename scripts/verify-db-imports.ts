...
class DatabaseImportVerifier {
      ...
      lines.forEach((line, index) => {
        ...
        if (line.includes("import { db }") || line.includes("import db ")) {
          this.violations.push({
            ...
            description:
              'Use "import { getDb } from "@/server/db/client"" instead of direct db import',
            severity: "error",
          });
        }
        ...
        if (line.includes('from "@/server/db/schema"')) {
          ...
          if (!filePath.includes("/api/") && !filePath.includes("/server/")) {
            this.violations.push({
              ...
              description:
                "Database schema imports should only be in server-side code. Use DTOs in components.",
              severity: "warning",
            });
          }
        }
  ...
  private reportResults(): void {
    ...
    // Group by rule
    const byRule = this.violations.reduce(
      (acc, violation) => {

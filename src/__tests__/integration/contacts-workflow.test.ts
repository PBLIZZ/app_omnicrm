...
describe("Contacts End-to-End Workflow Integration", () => {
  ...
  it("should complete full contact lifecycle: create, list, and verify data flow", async () => {
    ...
    fakes.contacts.createContact.mockResolvedValueOnce(createdContact);

    const result = await vi.mocked(contactsService.createContactService)(
      testUserId,
      contactInput as any,
    );
    ...
    if (result.success) {
      expect(result.data.displayName).toBe("Integration Test Contact");
      expect(result.data.primaryEmail).toBe("integration@test.com");
    ...
    fakes.contacts.createContact.mockResolvedValueOnce(repoContact);
    const repoResult = await fakes.contacts.createContact(testUserId, {
      displayName: "Direct Repo Contact",
      primaryEmail: "repo@test.com",
      primaryPhone: null,
      source: "gmail_import",
    ...
    fakes.contacts.listContacts.mockResolvedValueOnce({
      items: [createdContact, repoContact],
      total: 2,
    });

    const serviceResult = await vi.mocked(contactsService.listContactsService)(
      testUserId,
      listParams,
    );
    ...
    if (serviceResult.success) {
      expect(serviceResult.data.items).toHaveLength(2);
      expect(serviceResult.data.total).toBe(2);

      const contactNames = serviceResult.data.items
        .map((c: ContactListItem) => c.displayName)
        .sort();
    ...
    const searchResult = await vi.mocked(contactsService.listContactsService)(
      testUserId,
      searchParams,
    );
    ...
    if (searchResult.success) {
      ...
      expect(searchResult.data.items[0].displayName).toBe("Integration Test Contact");
    }
    ...
    const repoListResult = await fakes.contacts.listContacts(testUserId, listParams);

    expect(repoListResult.items).toHaveLength(2);
    expect(repoListResult.total).toBe(2);

    // Step 6: Verify data integrity across layers
    const serviceContact = serviceResult.success
      ? serviceResult.data.items.find(
          (c: ContactListItem) => c.displayName === "Integration Test Contact",
        )
      : null;
    const repoContactMatch = repoListResult.items.find(
      (c: ContactListItem) => c.displayName === "Integration Test Contact",
    );
    ...
    fakes.contacts.createContact.mockResolvedValueOnce(edgeCaseContact);
    ...
    if (edgeResult.success) {
      expect(edgeResult.data.primaryEmail).toBeNull();
      expect(edgeResult.data.primaryPhone).toBeNull();
    }
    ...
    const finalResult = await vi.mocked(contactsService.listContactsService)(
      testUserId,
      listParams,
    );
    expect(finalResult.success).toBe(true);
    if (finalResult.success) {
      expect(finalResult.data.total).toBe(3);
    }
  });

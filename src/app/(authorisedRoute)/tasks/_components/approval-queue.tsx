...
function TaskApprovalCard({ 
  ...
  const handleApprove = () => {
    onApprove(task.id, notes || undefined);
    setNotes("");
    setShowNotes(false);
  };

  const handleReject = () => {
    onReject(task.id, notes || undefined);
    setNotes("");
    setShowNotes(false);
  };
              ...
              {task.aiContext && (
                <div className="bg-blue-50 dark:bg-blue-950/20 p-3 rounded-md mb-3">
                  ...
                  <p className="text-xs text-blue-600 dark:text-blue-400">
                    {typeof task.aiContext === 'string' ? task.aiContext : JSON.stringify(task.aiContext)}
                  </p>
                </div>
              )}
...
export function ApprovalQueue({ open, onOpenChange, pendingTasks }: ApprovalQueueProps): JSX.Element {
  ...
  const handleApprove = (taskId: string, notes?: string) => {
    approveMutation.mutate({ taskId, notes });
  };

  const handleReject = (taskId: string, notes?: string) => {
    rejectMutation.mutate({ taskId, notes });
  };

  const handleBulkApprove = () => {
    selectedTasks.forEach(taskId => {
      approveMutation.mutate({ taskId, notes: bulkNotes || undefined });
    });
    setSelectedTasks([]);
    setBulkNotes("");
  };
  ...
  const handleBulkReject = () => {
    selectedTasks.forEach(taskId => {
      rejectMutation.mutate({ taskId, notes: bulkNotes || undefined });
    });
    setSelectedTasks([]);
    setBulkNotes("");
  };

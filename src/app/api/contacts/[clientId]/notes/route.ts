...
import { getClientNotes, createClientNote } from "@/server/services/contacts.service";
import {
  GetNotesQuerySchema,
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  const handler = handleGetWithQueryAuth(
    ...
    NotesListResponseSchema,
    async (query, userId) => {
      return await getClientNotes(userId, params.clientId);
    },
  );
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  const handler = handleAuth(
    ...
    CreatedNoteResponseSchema,
    async (data, userId) => {
      return await createClientNote(userId, params.clientId, data);
    },
  );

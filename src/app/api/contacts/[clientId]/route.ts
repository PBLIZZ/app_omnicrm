...
export async function GET(request: NextRequest, { params }: RouteParams) {
  const handler = handleAuth(z.void(), ContactResponseSchema, async (_, userId) => {
    const omniClient = await getOmniClient(userId, params.clientId);
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  const handler = handleAuth(
    ...
    async (data, userId) => {
      ...
      const omniClient = await updateOmniClient(userId, params.clientId, dataWithId);
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  const handler = handleAuth(z.void(), DeleteContactResponseSchema, async (_, userId) => {
    const deleted = await deleteOmniClient(userId, params.clientId);

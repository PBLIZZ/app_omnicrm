...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { timeRangeHours, includeResolved, provider, stage, severityFilter, includeDetails } =
      validation.data;
    ...
    try {
      ...
      const urgencyScore = calculateUrgencyScore({
        ...
        hasNetworkErrors: filteredRecentErrors.some(
          (e) => e.classification?.category === ERROR_CATEGORIES.NETWORK,
        ),
        hasQuotaErrors: filteredRecentErrors.some(
      ...
      if (includeDetails) {
        errorDetails = {
          recentErrors: filteredRecentErrors.slice(0, 10).map((error) => ({
            ...
            userMessage:
              error.classification?.recoveryStrategies &&
              error.classification.recoveryStrategies.length > 0
                ? (error.classification.recoveryStrategies[0]?.description ??
            ...
            userMessage:
              ...
              error.classification.recoveryStrategies.length > 0
                ? (error.classification.recoveryStrategies[0]?.description ??
                  "No recovery information available")
                : "No recovery information available",
            impact: error.classification?.severity,
...
function calculateUrgencyScore(metrics: {
  ...
  // Recent failure rate
  if (metrics.recentFailureRate > 5)
    score += 20; // More than 5 errors per hour
...
function generateErrorRecommendations(context: RecommendationContext): string[] {
  ...
  if (context.errorSummary.retryableErrors.length > 0) {
    recommendations.push(
      `🔄 ${context.errorSummary.retryableErrors.length} errors can be automatically retried`,
    );
  }
...
function getPatternDescription(category: string, count: number): string {
  ...
  return (
    ...
    `${count} ${category} error${count !== 1 ? "s" : ""}`
  );
}
...
function getPatternSuggestion(category: string): string {
  ...
  return (
    ...
    "Review error details for specific resolution steps"
  );
}

...
export function IntelligentProcessingApproval({
  ...
  const handleTaskApprovalChange = (itemId: string, taskId: string, approved: boolean) => {
    setApprovalData((prev) => ({
      ...
      [itemId]: {
        ...prev[itemId],
        approvedTasks: prev[itemId].approvedTasks.map((task: any) =>
  ...
  const handleProjectApprovalChange = (itemId: string, projectId: string, approved: boolean) => {
    setApprovalData((prev) => ({
      ...
      [itemId]: {
        ...prev[itemId],
        approvedProjects: prev[itemId].approvedProjects.map((project: any) =>
  ...
  const handleTaskModification = (itemId: string, taskId: string, field: string, value: any) => {
    setApprovalData((prev) => ({
      ...prev,
      [itemId]: {
        ...prev[itemId],
                        ...
                        <div className="flex-1 space-y-2">
                          <Input
                            value={
                              approvalData[item.inboxItem.id]?.approvedProjects?.find(
                                (p: any) => p.projectId === project.id,
                              )?.modifications?.name || project.name
                            }
                            onChange={(e) =>

...
> {
  return useMutation({
    mutationFn: async (contactId: string): Promise<ContactAIInsightResponse> => {
      return await fetchPost<ContactAIInsightResponse>(
        `/api/contacts-new/${contactId}/ai-insights`,
        {},
      );
    },
    onError: (error) => {
 ...
 */
export function useGenerateEmailSuggestion(): UseMutationResult<
...
> {
  return useMutation({
    mutationFn: async ({
      ...
      return await fetchPost<ContactEmailSuggestion>(
        `/api/contacts-new/${contactId}/email-suggestion`,
        { purpose },
      );
    },
    onError: (error) => {
 ...
 */
export function useGenerateNoteSuggestions(): UseMutationResult<
...
> {
  return useMutation({
    mutationFn: async (contactId: string): Promise<ContactNoteSuggestion[]> => {
      const data = await fetchPost<{ suggestions: ContactNoteSuggestion[] }>(
        `/api/contacts-new/${contactId}/note-suggestions`,
        {},
      );
 ...
 */
export function useGenerateTaskSuggestions(): UseMutationResult<
  ContactTaskSuggestion[],
  Error,
  string,
  unknown
> {
  return useMutation({
    mutationFn: async (contactId: string): Promise<ContactTaskSuggestion[]> => {
      const data = await fetchPost<{ suggestions: ContactTaskSuggestion[] }>(
        `/api/contacts-new/${contactId}/task-suggestions`,
        {},
      );
      return data.suggestions;
    },
 ...
 */
export function useCreateContactTask(): UseMutationResult<
  unknown,
  Error,
  {
    contactId: string;
...
> {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      return await fetchPost(`/api/contacts-new/${contactId}/tasks`, {
        title,
        description,
        priority,
        estimatedMinutes,
 ...
 */
export function useCreateNoteFromSuggestion(): UseMutationResult<
  unknown,
  Error,
  { contactId: string; content: string },
  unknown
 ...
 */
export function useCreateTaskFromSuggestion(): UseMutationResult<
  unknown,
  Error,
  {
    contactId: string;

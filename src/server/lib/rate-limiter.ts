...
export class RateLimiter {
   ...
   */
  static checkRateLimit(operation: string, userId: string): {
    allowed: boolean;
    resetTime?: number;
    remaining?: number;
    reason?: string;
    ...
    if (!config) {
      log.warn({ 
        op: "rate_limiter.unknown_operation", 
        operation, 
        userId 
      }, "Unknown operation for rate limiting");
      return { allowed: true };
    }
    ...
    if (existing.blocked) {
      log.warn({
        op: "rate_limiter.blocked_user",
        operation,
        userId,
        resetTime: existing.resetTime,
      }, "Rate limited user attempted blocked operation");
    ...
    if (existing.count >= config.maxRequests) {
      ...
      log.warn({
        op: "rate_limiter.limit_exceeded",
        operation,
        userId,
        count: existing.count,
        maxRequests: config.maxRequests,
        resetTime: existing.resetTime,
      }, "User exceeded rate limit and has been blocked");
   ...
   */
  static blockUser(operation: string, userId: string, durationMs: number = 60 * 60 * 1000): void {
    const config = RATE_LIMITS[operation];
    if (!config) return;
    ...
    log.warn({
      op: "rate_limiter.manual_block",
      operation,
      userId,
      durationMs,
    }, "User manually blocked from operation");
  }

  /**
   * Get current rate limit status for a user/operation
   */
  static getStatus(operation: string, userId: string): {
    remaining: number;
    resetTime: number;
    blocked: boolean;
  } | null {
   ...
   */
  static clearLimit(operation: string, userId: string): void {
    const config = RATE_LIMITS[operation];
    if (!config) return;
    ...
    log.info({
      op: "rate_limiter.limit_cleared",
      operation,
      userId,
    }, "Rate limit manually cleared");
  }
}
...
export function withRateLimit(operation: string) {
  return function<T extends unknown[], R>(
    ...
    return async function(userId: string, ...args: unknown[]): Promise<R> {
      const result = RateLimiter.checkRateLimit(operation, userId);

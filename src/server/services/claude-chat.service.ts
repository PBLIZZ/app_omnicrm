...
import {
  ...
  ANTHROPIC_MODEL,
} from "@/server/providers/anthropic.provider";
      ...
      async start(controller) {
        try {
          ...
          const isDatabaseQuery =
            ClaudeChatService.isDatabaseRelatedQuery(message) ??
          ...
          if (isDatabaseQuery) {
            ...
            if (queryResult.success && queryResult.data) {
              ...
              const hasMessage = (data: DatabaseQueryData): data is { message: string } => {
                return (
                  !!data &&
                  typeof data === "object" &&
                  "message" in data &&
              ...
              responseContent =
                queryResult.error ?? "Sorry, I couldn't process your database query.";
            }
            ...
            await chatStorage.createMessage(threadId, userId, "assistant", {
...
- Make emails scannable with good structure`;
    ...
    const emailContent =
      firstContent && firstContent.type === "text" && "text" in firstContent
        ? firstContent.text
        : "";
    return emailContent;
  }

...
export class ContactIntelligenceService {
   ...
   */
  private static async getContactEvents(userId: string, contactEmail: string) {
    const db = await getDb();
    const results = await db.execute(sql`
      ...
      WHERE ce.user_id = ${userId}
        ...
        AND ce.attendees::text LIKE ${`%${contactEmail}%`}
  ...
  private static analyzeEventPatterns(events: any[]) {
    ...
    const eventTypes = events.map(e => this.extractEventType(e.title, e.description)).filter(Boolean);
    const businessCategories = events.map(e => this.extractBusinessCategory(e.title, e.description)).filter(Boolean);
    ...
    const firstEventDate = events.length > 0 ? new Date(events[events.length - 1].start_time) : null;
    const lastEventDate = events.length > 0 ? new Date(events[0].start_time) : null;
    
    const relationshipDays = firstEventDate && lastEventDate 
  ...
  private static async generateAIAnalysis(
    ...
    const recentEvents = eventDetails.slice(0, 5).map(e => 
      `"${e.title}" (${e.type}) on ${e.date.toLocaleDateString()}`
    ).join('\n');
...
CONTACT ANALYSIS:
Email: ${contactEmail}
Relationship Duration: ${patterns.relationshipDays} days (${new Date(patterns.firstEventDate).toLocaleDateString()} to ${new Date(patterns.lastEventDate).toLocaleDateString()})
...
`;
   ...
   */
  private static getBasicInsights(patterns: any): ContactInsightsWithNote {
    ...
    if (patterns.eventTypes.some((t: string) => t?.toLowerCase().includes('yoga'))) tags.push('Yoga');
    if (patterns.eventTypes.some((t: string) => t?.toLowerCase().includes('massage'))) tags.push('Massage');
    if (patterns.eventTypes.some((t: string) => t?.toLowerCase().includes('meditation'))) tags.push('Meditation');
    if (patterns.eventTypes.some((t: string) => t?.toLowerCase().includes('pilates'))) tags.push('Pilates');
    if (patterns.eventTypes.some((t: string) => t?.toLowerCase().includes('workshop'))) tags.push('Workshops');
    if (patterns.eventTypes.some((t: string) => t?.toLowerCase().includes('private'))) tags.push('Private Sessions');
    if (patterns.eventTypes.some((t: string) => t?.toLowerCase().includes('class'))) tags.push('Group Classes');
    
    // Business category tags
    if (patterns.businessCategories.some((c: string) => c?.toLowerCase().includes('wellness'))) tags.push('Stress Relief');
    if (patterns.businessCategories.some((c: string) => c?.toLowerCase().includes('fitness'))) tags.push('Strength Building');
   ...
   */
  private static analyzeTimePatterns(events: any[]): { preferredTimes: string[], consistency: string } {
    const times = events.map(e => e.date.getHours());
    const days = events.map(e => e.date.getDay());
    
    // Categorize times
    const timeCategories: { [key: string]: number } = {};
    times.forEach(hour => {
      if (hour < 10) timeCategories['Early Morning'] = (timeCategories['Early Morning'] || 0) + 1;
      else if (hour < 12) timeCategories['Late Morning'] = (timeCategories['Late Morning'] || 0) + 1;
    ...
    const totalDays = events.length > 0 ? 
      Math.ceil((new Date(Math.max(...events.map(e => e.date.getTime()))).getTime() - 
                 new Date(Math.min(...events.map(e => e.date.getTime()))).getTime()) / (1000 * 60 * 60 * 24)) : 0;
   ...
   */
  private static analyzeServicePreferences(events: any[]): { primaryServices: string[], preferences: string[] } {
   ...
   */
  private static extractBusinessCategory(title: string, description?: string): string {
    const text = `${title} ${description || ''}`.toLowerCase();

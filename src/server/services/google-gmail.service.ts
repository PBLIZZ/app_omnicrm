    ...
    try {
      ...
      if (tokenExpiresIn < 300000 && refreshToken) {
        try {
            ...
            await db
              ...
              .where(
                and(
                  eq(userIntegrations.userId, userId),
                  eq(userIntegrations.provider, "google"),
                  eq(userIntegrations.service, integration.service ?? "gmail"),
                ),
              );
          ...
          if (
            refreshMsg.includes("invalid_grant") ||
      ...
      throw new GmailAuthError(errorMsg, "auth_failed", true);
    }
  }

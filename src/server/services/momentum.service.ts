...
export class MomentumService {
  ...
  async getProjects(userId: string, filters: ProjectFilters = {}): Promise<ProjectDTO[]> {
    ...
    if (filters.dueAfter) {
      filteredProjects = filteredProjects.filter(p =>
        p.dueDate && new Date(p.dueDate) >= filters.dueAfter!
      );
    }

    if (filters.dueBefore) {
      filteredProjects = filteredProjects.filter(p =>
        p.dueDate && new Date(p.dueDate) <= filters.dueBefore!
      );
    }
  ...
  async updateProject(projectId: string, userId: string, data: UpdateProjectDTO): Promise<ProjectDTO | null> {
  ...
  async createQuickTask(userId: string, data: QuickTaskCreateDTO): Promise<TaskDTO> {
  ...
  async getTasks(userId: string, filters: TaskFilters = {}): Promise<TaskDTO[]> {
    ...
    if (filters.search) {
      ...
      filteredTasks = filteredTasks.filter(t => {
        ...
        const hasDescriptionMatch = t.details &&
    ...
    if (filters.dueAfter) {
      filteredTasks = filteredTasks.filter(t =>
        t.dueDate && new Date(t.dueDate) >= filters.dueAfter!
      );
    }

    if (filters.dueBefore) {
      filteredTasks = filteredTasks.filter(t =>
        t.dueDate && new Date(t.dueDate) <= filters.dueBefore!
      );
    }
  ...
  async getProjectTasks(projectId: string, userId: string): Promise<TaskDTO[]> {
  ...
  async getGoals(userId: string, filters: GoalFilters = {}): Promise<GoalDTO[]> {
    ...
    if (filters.search) {
      ...
      filteredGoals = filteredGoals.filter(g => {
        ...
        const hasDescriptionMatch = g.details &&
    ...
    if (filters.targetAfter) {
      filteredGoals = filteredGoals.filter(g =>
        g.targetDate && new Date(g.targetDate) >= filters.targetAfter!
      );
    }

    if (filters.targetBefore) {
      filteredGoals = filteredGoals.filter(g =>
        g.targetDate && new Date(g.targetDate) <= filters.targetBefore!
      );
    }
  ...
  async updateDailyPulseLog(logId: string, userId: string, data: UpdateDailyPulseLogDTO): Promise<DailyPulseLogDTO | null> {

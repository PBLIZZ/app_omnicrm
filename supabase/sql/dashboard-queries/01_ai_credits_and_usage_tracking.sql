...
create policy ai_quotas_rw_own on ai_quotas for all to authenticated using (user_id = auth.uid ())
with
...
create policy ai_usage_rw_own on ai_usage for all to authenticated using (user_id = auth.uid ())
with
  check (user_id = auth.uid ());
